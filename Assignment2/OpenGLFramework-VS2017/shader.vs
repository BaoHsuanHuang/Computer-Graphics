...
// in : each vertex has 3 attributions (position, color, normal)
// out : data that will pass to fragment shader
// one uniform variable (with the same name) can be used in serviral shader files

layout (location = 0) in vec3 aPos;  // aPos variable has attribute position 0 // vbo
layout (location = 1) in vec3 aColor; // p_color
layout (location = 2) in vec3 aNormal; // p_normal
